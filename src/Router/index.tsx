...
const LAZY_RENDER_COMPONENTS: {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    [_: string]: React.LazyExoticComponent<ComponentType<any>>;
} = {
...
export default function RouterComponent() {
    ...
    useEffect(() => {
        const isUnAuthRoute = NO_AUTH_ROUTES.some(
    ...
    return (
        <Routes>
            <Route path="/">
                <Route path="/">{RenderAuthRoute(AUTH_ROUTES)}</Route>
                <Route path="/public">{RenderAuthRoute(PUBLIC_ROUTES)}</Route>
                <Route path="/auth">{RenderUnAuthRoute(NO_AUTH_ROUTES)}</Route>
